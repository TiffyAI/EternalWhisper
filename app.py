...
def search_serpapi(query):
    """Use SerpApi, prepend 'What a sexy, spunky girl thinks when asked', append '...' to last word, split at commas."""
    try:
        # Prepend sexy prefix and append "..." to last word
        words = query.split()
        if words:
            words[-1] = words[-1] + "..."
            query = f"What a sexy, spunky girl thinks when asked {' '.join(words)}"
        params = {
            "q": query,
            "engine": "google",
