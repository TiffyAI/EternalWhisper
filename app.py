...
def process_query(query):
    ...
    try:
        ...
        lines = [line.strip() for line in content.split('\n') if any(word in line.lower() for word in query.lower().split()) and not any(n in line.lower() for n in ['cookie', 'microsoft.com'])]
        scan_resp = f"Essence caught: {' '.join(lines[:3])}" if lines else f"Whispers faint: {query}..."
    ...
    <body>
        ...
        <script>
            ...
            function dissectAndSpeak(userQuery, rawResp) {
                ...
                const essenceLines = essenceMatch ? essenceMatch[1].split('\n').filter(l => l.trim() && !l.includes('https://') && !l.includes('cookie') && !l.includes('microsoft.com')) : [];
