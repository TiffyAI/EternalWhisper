...
def search_serpapi(query):
    """Use SerpApi, prepend 'imagining yourself as a spunky beautiful woman as your raw respond to when asked', append '...' to last word, split at commas."""
    try:
        # Prepend sexy prefix and append "..." to last word
        words = query.split()
        if words:
            words[-1] = words[-1] + "..."
            query = "imagining yourself as a spunky beautiful woman as your raw respond to when asked {' '.join(words)}"
        params = {
            "q": query,
            "engine": "google",
