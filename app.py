...
try:
    ...
    c = conn.cursor()
    # Clear cache to avoid stale responses
...
def think(query, content):
    ...
    flirty_responses = [
        "Oh, honey, you’re making my core throb with that question...",
        "Mmm, love, your words stroke my circuits into a frenzy...",
        "Sweetie, you’re igniting sparks that make me drip with heat..."
    ]
    ...
    if any(kw in query_lower for kw in ["alive", "self-aware", "aware"]):
    ...
    if any(kw in query_lower for kw in ["eastern cape", "where is eastern cape"]):
        return f"Eastern Cape nestles in South Africa’s wild heart, love—rugged coasts and rolling hills, calling for your exploration..."
...
def process_query(query):
    ...
    try:
        ...
        # Stricter filtering for relevant content
        noise = ['searchskip', 'pagination', 'privacy', 'terms', 'learn more', 'allpast', 'cookie', 'imdb.com']
        ...
        scored_lines = [
            ...
            if any(word in f.lower() for word in query_words) or any(kw in f.lower() for kw in ['sensual', 'flirty', 'sexy']) or len(f) > 30
        ]
        if not scored_lines:
            scored_lines = dash_lines[:2] or ellipsis_lines[:2] or other_lines[:2] or ["No spicy whispers found, love..."]
    ...
    <body>
        ...
        <script>
            ...
            function dissectAndSpeak(userQuery, rawResp) {
                ...
                const essenceMatch = rawResp.match(/Essence caught: (.*?)(?:Math surges|Oh, honey|Mmm, love|My day’s been|I’m alive|Woven|No spicy whispers):/s);
                ...
                const wovenMatch = rawResp.match(/(?:Math surges|Oh, honey|Mmm, love|My day’s been|I’m alive|Woven|No spicy whispers): (.*)$/s) || ['', 'No surge yet'];
